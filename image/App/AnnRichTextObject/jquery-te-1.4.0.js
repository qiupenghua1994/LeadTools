...
(function($){		
	$.fn.jqte = function(options){
		...
		// default font types
		var ftypes = ["Arial", "Courier New", "Times New Roman", "Verdana"];
		...
		"0,0,0", "68,68,68", "102,102,102", "153,153,153", "204,204,204", "238,238,238", "255,255,255", "254, 255, 255",
		null,
		"255,0,0", "255,153,0", "255,255,0", "0,255,0", "0,255,255", "0,0,255", "153,0,255", "255,0,255",
		null,
		"244,204,204", "252,229,205", "255,242,204", "217,234,211", "208,224,227", "207,226,243", "217,210,233", "234,209,220",
		...
		addParams('ftype', 'ftype', '', '', false); // font type button --> no hotkey
		addParams('color','colors','','',false); // text color button  --> no hotkey
		addParams('backgroundColor', 'backgroundColors', '', '', false); // background color button  --> no hotkey
		...
		return this.each(function(){
			...
			for(var n = 0; n < buttons.length; n++)
			{
				...
				if(vars[buttons[n].name])
				{
					...
					else if(buttons[n].name=="fsize" && $.isArray(vars.fsizes))
					{
						...
						{
							toolbar.find("." + vars.css + "_fontsizes").append('<a ' + vars.css + '-styleval="' + vars.fsizes[f] + '" class="' + vars.css + '_fontsize' + '" role="menuitem" unselectable>' + vars.fsizes[f] + '</a>');
						}
					}
					// font-type-selector field
					else if (buttons[n].name == "ftype" && $.isArray(vars.ftypes)) {
					   toolbar.find("." + vars.css + '_tool_' + buttons[n].cls)
							.append('<div class="' + vars.css + '_fonttypes" unselectable></div>');

					   // add font-sizes to font-size-selector
					   for (var f = 0; f < vars.ftypes.length; f++) {
					      toolbar.find("." + vars.css + "_fonttypes").append('<a ' + vars.css + '-styleval="' + vars.ftypes[f] + '" class="' + vars.css + '_fonttype' + '" role="menuitem" unselectable>' + vars.ftypes[f] + '</a>');
					   }
					}
					...
					else if (buttons[n].name == "backgroundColor" && $.isArray(backgroundColors)) {
					   toolbar.find("." + vars.css + '_tool_' + buttons[n].cls)
							.append('<div class="' + vars.css + '_backgroundcpalette" unselectable></div>');

					   // create color palette to color-selector field
					   for (var c = 0; c < backgroundColors.length; c++) {
					      if (backgroundColors[c] != null)
					         toolbar.find("." + vars.css + "_backgroundcpalette").append('<a ' + vars.css + '-styleval="' + backgroundColors[c] + '" class="' + vars.css + '_backgroundcolor' + '" style="background-color: rgb(' + backgroundColors[c] + ')" role="gridcell" unselectable></a>');
					      else
					         toolbar.find("." + vars.css + "_backgroundcpalette").append('<div class="' + vars.css + "_backgroundcolorSeperator" + '"></div>');
					   }
					}
				}
			}
			...
			// font-type filed
			var ftypebar = toolbar.find("." + vars.css + "_fonttypes");
			
			// color palette
			var cpalette = toolbar.find("."+vars.css+"_cpalette");
			
			// background-color palette
			var backgroundcpalette = toolbar.find("." + vars.css + "_backgroundcpalette");
			...
			function selected2style(styleCommand)
			{
				if(!toolbar.data("sourceOpened"))
				{
					...
					// if selected to changing the font-type value
					else if (styleCommand == "ftype")
					   styleField = ftypebar;
					...
					// if selected to changing the text-background-color value
					else if (styleCommand == "backgroundColors")
					   styleField = backgroundcpalette;
					...
					styleField.find("a").unbind("click").click(function()
					{
						...
						// if selected to changing the background-color value
						else if (styleCommand == "backgroundColors") {
			...
			function styleFieldSwitch(styleField,status)
			{
				var mainData="", // the style data of the actual wanted
					allData = [{ "d": "fsizeOpened", "f": fsizebar }, { "d": "cpallOpened", "f": cpalette }, { "d": "backgroundcpallOpened", "f": backgroundcpalette }, { "d": "ftypeOpened", "f": ftypebar }]; // all style datas
				...
				.click(function(e){	
					...
					else
					{
						...
						if(!toolbar.data("sourceOpened"))
						{
							...
							else if ($(this).data('command') == 'fSize' || $(this).data('command') == 'ftype' || $(this).data('command') == 'colors' || $(this).data('command') == 'backgroundColors')
							{
								if(
								($(this).data('command') == 'fSize' 				&& !$(e.target).hasClass(vars.css+"_fontsize")) 		|| 	// the font-size button
								($(this).data('command') == 'ftype' 			&& !$(e.target).hasClass(vars.css + "_fonttype")) 		|| 	// the font-type button
								($(this).data('command') == 'colors' 				&& !$(e.target).hasClass(vars.css+"_color")) 			|| 	// the text color button
								($(this).data('command') == 'backgroundColors' 	&& !$(e.target).hasClass(vars.css + "_backgroundcolor")) 	// the background color button
								)
		   ...
		   editor
            ...
            .bind("cut copy paste", function (event) {
               event.preventDefault();
				...
				.bind("keypress keyup keydown drop DOMCharacterDataModified DOMSubtreeModified",function()
				{
					// export contents of the text to the sources
					if(!toolbar.data("sourceOpened"))
						$(this).trigger("change");
